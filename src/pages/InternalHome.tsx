      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          { (userRole === UserRole.ADMIN || userRole === UserRole.MANAGEMENT) && (
            <Button size="sm" onClick={() => setEditingComp({} as any)} className="bg-indigo-600 hover:bg-indigo-700 ml-4">
              <Plus className="w-4 h-4 mr-1" /> Input Lomba
            </Button>
          )}
...
<div className="mb-8 bg-white p-6 rounded-xl border shadow-sm space-y-4">
      ...
      <Dialog open={!!editingComp} onOpenChange={(open) => !open && setEditingComp(null)}>
        <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Input Lomba</DialogTitle>
          </DialogHeader>
          {editingComp && (
            <CompetitionForm 
              initialData={editingComp.id ? editingComp : undefined}
              onSuccess={() => {
                setEditingComp(null);
                // Need to reload data here - simple reload for now
